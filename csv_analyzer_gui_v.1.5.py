...
class CSVAnalyzerGUI:
    def __init__(self, master):
        ...
        self.results_tree.tag_configure('header', font=('Arial', 10, 'bold'))
        self.results_tree.tag_configure('odd', background='#f0f0ff')
        self.results_tree.tag_configure('even', background='#e0e0ff')
    ...
    def analyze_csv(self):
        ...
        for i in numerical_columns:
            ...
            # Add header row for column statistics
            self.results_tree.insert("", tk.END, values=(f"Statistics for column '{header[i]}'", "", "", "", "", "", ""), tags=('header',))
            ...
            if i % 2 == 0:
                self.results_tree.insert("", tk.END, values=("", average, minimum, maximum, median, mode, std_dev), tags=('even',))
            else:
                self.results_tree.insert("", tk.END, values=("", average, minimum, maximum, median, mode, std_dev), tags=('odd',))
        ...
        if date_column:
            try:
                ...
                if date_list:
                    ...
                    # Add header row for date analysis
                    self.results_tree.insert("", tk.END, values=(f"Date Analysis for column '{date_column}'", "", "", "", "", "", ""), tags=('header',))

                     # Add data row with alternating background colors
                    if date_index % 2 == 0:
                        self.results_tree.insert("", tk.END, values=("", f"Oldest Date = {oldest_date}",  f"Newest Date = {newest_date}", "", "", "", ""), tags=('even',))
                    else:
                        self.results_tree.insert("", tk.END, values=("", f"Oldest Date = {oldest_date}",  f"Newest Date = {newest_date}", "", "", "", ""), tags=('odd',))
                else:
